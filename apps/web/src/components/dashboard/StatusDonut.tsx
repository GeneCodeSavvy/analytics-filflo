...
export function StatusDonut({
  ...
  return (
    <section className={`${chartPanelClass} flex flex-col [animation-delay:80ms]`}>
      <h2 className="mb-2 text-[16px] font-semibold text-[--ink-1]">
        Status donut
      </h2>
      <div className="flex flex-1 items-center justify-center gap-4">
        <div className="relative h-[180px] w-[180px] shrink-0 cursor-crosshair">
        ...
        <div className="flex flex-col gap-2">
          {slices.map((slice) => (
            ...
            >
              ...
              />
              <span className="w-[80px]">{slice.status.replace("_", " ")}</span>
              <span className="w-4 text-right text-[--ink-1]">{slice.count}</span>
