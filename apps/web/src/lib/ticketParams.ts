...
export function mergeFilters(params: URLSearchParams, patch: Partial<TicketFilters>): URLSearchParams {
...
export function mergeSort(params: URLSearchParams, sort: TicketSort[]): URLSearchParams {
  const next = new URLSearchParams(params);
  if (sort.length === 0) {
    next.delete('sort');
  } else {
...
export function parseSort(params: URLSearchParams): TicketSort[] {
  ...
  return sortStr.split(',').map((item) => {
    const [field, dir] = item.split(':') as [TicketSort['field'], TicketSort['dir']];
    return { field, dir };
  });
}
